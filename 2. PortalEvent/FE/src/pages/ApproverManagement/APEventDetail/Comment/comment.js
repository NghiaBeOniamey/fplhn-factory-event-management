...
import "../../APEventDetail/Comment/comment.css";
...
const CommentSection = (props) => {
  ...
  useEffect(() => {
    APEventDetailApi.getCommentByIdEvent(props.value, currentPage).then(
      (res) => {
  ...
  const handleSend = (idComment, commentIndex) => {
    ...
    APEventDetailApi.postReplyComment({
      eventId: props.value,
  ...
  const handleDelete = (commentID) => {
    APEventDetailApi.deleteComment({
      commentId: commentID,
      // userId: "",
    })
      .then((res) => {
