...
export class ADRegistrationListApi {
    // lấy ra danh sách người đăng ký
    static fetchAll = (data) => {
        return request({
            method: "GET",
            url: URL_API_ADMIN_MANAGEMENT + `/registration-list`,
            params: data
        });
    };
    ...
    static getListExport = (id, data) => {
        return request({
            method: "GET",
            url: URL_API_ADMIN_MANAGEMENT + `/registration-list/${id}`,
            params: {
    ...
    static handleExportRegistration = (eventId) => {
        ...
        fetch(URL_API_ADMIN_MANAGEMENT + `/registration-list/export-registration?${params.toString()}`, {
            method: "POST",
            headers: {
