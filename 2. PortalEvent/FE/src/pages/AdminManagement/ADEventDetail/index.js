...
import { ADEventDetailApi } from "./ADEventDetailApi";
...
export default function EventDetail() {
    ...
    const detailEventInfo = (id) => {
        ADEventDetailApi.detailWaiting(id)
            .then((response) => {
    ...
    const handleOk = (id) => {
        setTimeout(() => {
            ...
            setErrMessage("");
            ADEventDetailApi.approvalEvent({
                id: id,
                reason: reason,
                status: 2,
            })
    ...
    const handleApprovalEvent = (id) => {
        ADEventDetailApi.countEventInTime(id, startTime.getTime(), endTime.getTime())
            .then((res) => {
                if (res.data.data) {
                    modalNoti.confirm({
                        title: "Cảnh báo!!!",
    ...
    const handleListOrganizerInEvent = (id) => {
        ADEventDetailApi.getListOrganizerByIdEvent(id)
            .then((response) => {
    ...
    const loadListAgendaItem = (idEvent) => {
        ADEventDetailApi.getAgendaItem(idEvent)
            .then((response) => {
    ...
    const getResources = (idEvent) => {
        ADEventDetailApi.getResoure(idEvent)
            .then((response) => {
    ...
    const getObjects = (idEvent) => {
        ADEventDetailApi.getObject(idEvent)
            .then((response) => {
    ...
    const getLocations = (idEvent) => {
        ADEventDetailApi.getLocation(idEvent)
            .then((response) => {
    ...
    const getEvidence = (idEvent) => {
        ADEventDetailApi.getEvidenceByIdEvent(idEvent)
            .then((response) => {
    ...
    const getMajors = (idEvent) => {
        ADEventDetailApi.getMajor(idEvent)
            .then((response) => {
    ...
    const approverPeriodicEvent = () => {
        ADEventDetailApi.approverPeriodicEvent(id).then(
            (response) => {
    ...
    const noApproverPeriodicEvent = () => {
        ...
        if (check === 0) {
            ADEventDetailApi.noApproverPeriodicEvent(id, reasonWait).then(
                (response) => {
                            ...
                            <Form.Item style={{ marginTop: 20 }}>
                                {status !== "Dự kiến tổ chức" &&
                                    ...
                                    status !== "Đã được bộ môn thông qua" && (
                                        <Link to={`/admin-management/registration-list/${id}`}>
                                            <Button
                                                type="primary"
                                                style={{
                                                    marginRight: 10,
                                ...
                                {status !== "Dự kiến tổ chức" &&
                                    ...
                                    status !== "Chờ phê duyệt" &&
                                    status !== "Đã được bộ môn thông qua" && (
                                        <Link to={`/admin-management/event-attendance/${id}`}>
                                            <Button
                                                style={{
                                                    marginRight: 10,
