import {
    ExclamationCircleFilled, FileDoneOutlined, UserOutlined
...
import { AdminHOEventDetailApi } from "./AdminHOEventDetailApi";
import "./index.css";
...
export default function EventDetail() {
    ...
    const detailEventInfo = (id) => {
        AdminHOEventDetailApi.detailWaiting(id)
            .then((response) => {
                ...
                if (obj.status === 0) {
                    ...
                    setStatus("Đã được bộ môn thông qua");
                    setStatusColor("status13Color");
                }
    ...
    const handleOk = (id) => {
        setTimeout(() => {
            ...
            setErrMessage("");
            AdminHOEventDetailApi.approvalEvent({
                id: id,
                reason: reason,
                status: 2,
            })
    ...
    const handleApprovalEvent = (id) => {
        AdminHOEventDetailApi.countEventInTime(
            id,
            startTime.getTime(),
            endTime.getTime()
        )
            .then((res) => {
                if (res.data.data) {
                    modalNoti.confirm({
                        ...
                        content: (
                            <p>
                                Thời gian diễn ra sự kiện này trùng với sự kiện khác đã được phê
                                duyệt:{" "}
                                <strong>
                                    <i>{res.data.data}</i>
                                </strong>
    ...
    const handleListOrganizerInEvent = (id) => {
        AdminHOEventDetailApi.getListOrganizerByIdEvent(id)
            .then((response) => {
    ...
    const loadListAgendaItem = (idEvent) => {
        AdminHOEventDetailApi.getAgendaItem(idEvent)
            .then((response) => {
    ...
    const getResources = (idEvent) => {
        AdminHOEventDetailApi.getResoure(idEvent)
            .then((response) => {
    ...
    const getObjects = (idEvent) => {
        AdminHOEventDetailApi.getObject(idEvent)
            .then((response) => {
    ...
    const getLocations = (idEvent) => {
        AdminHOEventDetailApi.getLocation(idEvent)
            .then((response) => {
    ...
    const getEvidence = (idEvent) => {
        AdminHOEventDetailApi.getEvidenceByIdEvent(idEvent)
            .then((response) => {
    ...
    const getMajors = (idEvent) => {
        AdminHOEventDetailApi.getMajor(idEvent)
            .then((response) => {
    ...
    const approverPeriodicEvent = () => {
        AdminHOEventDetailApi.approverPeriodicEvent(id).then(
            (response) => {
    ...
    const noApproverPeriodicEvent = () => {
        ...
        if (check === 0) {
            AdminHOEventDetailApi.noApproverPeriodicEvent(id, reasonWait).then(
                (response) => {
                ...
                <Form labelCol={{ span: 7 }}>
                                ...
                                <Row style={{ marginBottom: "15px" }}>
                                    <Col className={"textCenter"} span={24}>
                                        <b style={{ color: "red", fontSize: "large" }}>
                                            * Sự kiện gửi yêu cầu phê duyệt không đủ thời gian quy
                                            định
                                        </b>
                                    </Col>
                                </Row>
                    ...
                    <Row gutter={16}>
                        <Col span={11} className="col-left">
                            ...
                            <Form.Item label="Mục tiêu sự kiên">
                                <JoditEditor
                                    readOnly={true}
                                    value={description}
                                    onBlur={onchange}
                            ...
                            <Form.Item label="Bộ môn" style={{ marginBottom: 10 }}>
                                <ul>
                                    {departmentName
                                        ?
                        ...
                        <Col span={11}>
                            ...
                            <Form.Item label="Chuyên ngành" style={{ marginBottom: 10 }}>
                                <ul>
                                    {majorName
                                        ?
                                        majorName.split(",").map((item, index) => {
                                            return (
                                                <li key={index} className="bullet-list-item">
                                                    {item}
                                                </li>
                        ...
                        <Row justify="center">
                            <Form.Item style={{ marginTop: 20 }}>
                                {status !== "Dự kiến tổ chức" &&
                                    ...
                                    status !== "Đã được bộ môn thông qua" && (
                                        <Link to={`/admin-ho-management/registration-list/${id}`}>
                                            <Button
                                                type="primary"
                                                style={{
                                                    marginRight: 10,
                                ...
                                {status !== "Dự kiến tổ chức" &&
                                    ...
                                    status !== "Chờ phê duyệt" &&
                                    status !== "Đã được bộ môn thông qua" && (
                                        <Link to={`/admin-ho-management/event-attendance/${id}`}>
                                            <Button
                                                style={{
                                                    marginRight: 10,
                                ...
                                {/* {status !== "Đã tổ chức" && status !== "Đã phê duyệt" && status !== "Đã đóng" && status !== "Cần sửa"&& (
                                    <>
                                        <Button
                                ...
                                <ShowHistoryModal
                                    displayName={APPROVER_EVENT_DETAIL}
                                    eventId={id}
                                />
                            </Form.Item>
                        </Row>
