...
const APSEChartTop = ({semesterId, campusId, departmentId}) => {
    const [listTopEvent, setListTopEvent] = useState([]);

    useEffect(() => {
        if(semesterId !== "" && campusId !== "" && departmentId !== ""){
            APStatisticsEventApi.getTopEvent(semesterId, campusId, departmentId).then(
                (res) => {
    ...
    }, [semesterId, campusId, departmentId]);
...
export default memo(APSEChartTop);
