...
import { AdminHOCategoryApi } from "./AdminHOCategoryManagementApi";
import "./style.css";
import {
  faList,
  faMagnifyingGlass,
...
import { ADMIN_CATEGORY_MANAGEMENT } from "../../../constants/DisplayName";
...
export default function CategoryList() {
  ...
  const loadDataCategory = (page) => {
    AdminHOCategoryApi.categoryList(page, searchName)
      .then((response) => {
  ...
  //Search Thể loại
  const searchTheLoai = (page,searchName) => {
    AdminHOCategoryApi.categoryList(page, searchName)
      .then((response) => {
        setListCategory(response.data.data);
        setCurrentPage(0);
  ...
  const updateCategory = (id) => {
    ...
    AdminHOCategoryApi.updateCategory(id, obj)
      .then((response) => {
  ...
  const deleteCategory = (id) => {
    AdminHOCategoryApi.deleteCategory(id)
      .then((response) => {
  ...
  const detailCategory = (id) => {
    AdminHOCategoryApi.detailCategory(id)
      .then((response) => {
  ...
  const postCategory = () => {
    ...
    AdminHOCategoryApi.addCategory(obj)
      .then((response) => {
  ...
  const handleOk = () => {
    ...
    if (categoryName.length >= 255) {
      setCategoryNameError("Tên thể loại không được vượt quá 255 ký tự.");
      return;
    }
    if (modalAction === 1) {
    ...
    },
    {
      ...
      render: (_, record) => (
        <div className={"textCenter"}>
          ...
          {/* <Popconfirm
            title="Xóa Bộ môn"
            description="Bạn có chắc chắn muốn xóa bộ môn này không?"
            onConfirm={() => {
            ...
            cancelText="Không"
          >
            <Tooltip placement="top" title={"Xóa bộ môn"}>
              <Button
                key="delete"
                danger
                shape={"circle"}
        ...
        <div className="row">
          ...
          <div className="style-button">
            <Button
              type="primary"
              onClick={() => {
                searchTheLoai(currentPage,searchName);
              }}
            >
          ...
          <div className="button-category">
            ...
            <ShowHistoryModal displayName={ADMIN_CATEGORY_MANAGEMENT} />
          </div>
        </div>
