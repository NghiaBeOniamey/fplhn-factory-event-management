...
import "./index.css";
import { AdminHOEventWaitingApprovalApi } from "./AdminHOEventWaitingApprovalApi";
...
export default function EventWaitingApproval() {
    ...
    useEffect(() => {
        loadCategory();
        // loadParentMajorParticipants();
        // loadParentMajorParticipantsLecturer();
        getAllSemester();
        getAllCampus();
    }, []);
    ...
    const getAllCampus = async () => {
        try {
            const res = await APStatisticsEventApi.getAllCampus();
            setListCampus(res.data.data);
        } catch (e) {
            message.error("Không lấy được dữ liệu Campus!");
        }
    }

    // const loadParentMajorParticipants = () => {
    //     AdminHOEventWaitingApprovalApi.fetchAllParentMajor().then(response => {
    //         setParentMajorListParticipants(response.data);
    //     }).catch(e => {
    //         //  //console.log(e);
    //     })
    // }
    //
    // const loadParentMajorParticipantsLecturer = () => {
    //     AdminHOEventWaitingApprovalApi.fetchAllParentMajor().then(response => {
    //         setParentMajorListParticipantsLecturer(response.data);
    //     }).catch(e => {
    //         //  //console.log(e);
    //     })
    // }
    ...
    const handleOkParticipants = () => {
        AdminHOEventWaitingApprovalApi.handleExportParticipants(selectedMajorParticipants);
    ...
    const showModalParticipants = () => {
        setIsModalOpenParticipants(true);
    };

    const handleOkParticipantsLecturer = () => {
        AdminHOEventWaitingApprovalApi.handleExportParticipantsLecturer(selectedMajorParticipantsLecturer);
    ...
    const showModalParticipantsLecturer = () => {
        setIsModalOpenParticipantsLecturer(true);
    };

    const handleOkEvent = () => {
        AdminHOEventWaitingApprovalApi.handleExportEvent(idSemester, idCampus, idDepartment);
    ...
    const loadCategory = () => {
        AdminHOEventWaitingApprovalApi.fetchListCategory()
            .then((response) => {
    ...
    const loadListEvent = () => {
        AdminHOEventWaitingApprovalApi.fetchListEventWaiting({
            page: currentPage - 1,
            size: 5,
            name: null,
            eventGroup: null,
    ...
    const renderAction = (record) => (
        <div class="textCenter">
            <Tooltip title="Xem chi tiết" placement="top">
                <Link to={`/admin-ho-management/event-detail/${record.id}`}>
    ...
    return (
        <>
            <div className="rounded-lg shadow-md w-full bg-white p-8">
                <div style={{ marginTop: 5 }}>
                    <Card
                        ...
                        bordered={false}
                    >
                        <Form labelCol={{ span: 6 }} wrapperCol={{ span: 15 }} form={form}>
                            <Row gutter={16}>
                                <Col span={12}>
                                    ...
                                    >
                                        <Select
                                            ...
                                            placeholder="Chọn cơ sở tổ chức sự kiện ..."
                                            showSearch
                                            filterOption={(input, option) => (option?.label ?? '').toLowerCase().includes(input.toLowerCase())}
                                            options={listCampus?.map((item) => ({
                                                value: item.id,
                                                label: item.name
                                            }))}
                                        />
                                        {/* {listCampus.map((campus, index) => (
                                ...
                                <Col span={12}>
                                    ...
                                    >
                                        <Select
                                            ...
                                            placeholder="Chọn bộ môn tổ chức sự kiện ..."
                                            showSearch
                                            filterOption={(input, option) => (option?.label ?? '').toLowerCase().includes(input.toLowerCase())}
            ...
            >
                ...
                <Form.Item name={"mainMajor"} label={"Bộ môn"} labelCol={{ span: 8 }} wrapperCol={{ span: 15 }}>
                    <Select
                        className="me-2"
