...
const api = URL_API_ADMIN_HO_MANAGEMENT + `/event-approval`;

export class AdminHOEventWaitingApprovalApi {
    ...
    static handleExportParticipants = (code) => {
        fetch(api + `/export/participants?subjectCode=${code}`, {
            method: "GET",
            headers: {
    ...
    static handleExportParticipantsLecturer = (code) => {
        fetch(api + `/export/participants-lecturer?subjectCode=${code}`, {
            method: "GET",
            headers: {
                'Authorization': 'Bearer ' + Cookies.get('token'),
            },
    ...
    static handleExportEvent = (semesterId, campusId, departmentId) => {
