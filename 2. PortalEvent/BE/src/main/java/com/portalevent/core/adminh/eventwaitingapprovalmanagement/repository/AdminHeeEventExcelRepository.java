...
public interface AdminHeeEventExcelRepository extends EventRepository {
    ...
    @Query(value = """
            ...
            WHERE
             e.status != 1
             AND e.semester_id = :#{#adminHSDRequest.idSemester}
            AND c.code = :#{#request.currentTrainingFacilityCode}
            AND d.id = :#{#adminHSDRequest.idDepartment}
            GROUP BY
             d.name,
             e.name,
             e.description,
             ...
             e.status
            """, nativeQuery = true)
    List<AdminHeeEventExcelResponse> findAllEventExcel(AdminHSDRequest adminHSDRequest, TokenFindRequest request);
