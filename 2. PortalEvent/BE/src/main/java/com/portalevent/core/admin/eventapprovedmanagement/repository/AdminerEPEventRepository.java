package com.portalevent.core.admin.eventapprovedmanagement.repository;
...
@Repository
public interface AdminerEPEventRepository extends EventRepository {
            ...
            """, countQuery = """
                select count(e.id)
                     ...
                     where ( :#{#req.name} is null or e.name like :#{'%'+#req.name+'%'})
                       ...
                       and (:#{#request.currentTrainingFacilityCode} is null or e.training_facility_code like :#{#request.currentTrainingFacilityCode})
            """, nativeQuery = true)
    Page<AdminerEventApprovedResponse> getListEventAppoved(Pageable pageable, AdminerEventApprovedRequest req, TokenFindRequest request);
