package com.portalevent.core.adminh.eventdetailmanagement.service.impl;
...
import com.portalevent.core.adminh.eventdetailmanagement.repository.AdminHedAgendaItemRepository;
import com.portalevent.core.adminh.eventdetailmanagement.repository.AdminHedEventDetailRepository;
import com.portalevent.core.adminh.eventdetailmanagement.repository.AdminHedEventOrganizerRepository;
import com.portalevent.core.adminh.eventdetailmanagement.repository.AdminHedEvidenceRepository;
import com.portalevent.core.adminh.eventdetailmanagement.repository.AdminHedLocationRepository;
import com.portalevent.core.adminh.eventdetailmanagement.repository.AdminHedMajorRepository;
import com.portalevent.core.adminh.eventdetailmanagement.repository.AdminHedObjectRepository;
import com.portalevent.core.adminh.eventdetailmanagement.repository.AdminHedResourceRepository;
...
import com.portalevent.core.adminh.eventwaitingapprovalmanagement.repository.AdminHewCommentRepository;
...
@Service
@Slf4j
public class AdminHedEventDetailServiceImpl implements AdminHedEventDetailService {

    private final AdminHedEventDetailRepository eventRepository;

    private final AdminHewCommentRepository commentRepository;

    private final AdminHedAgendaItemRepository agendaItemRepository;

    private final AdminHedResourceRepository resourceReposiotry;

    private final AdminHedLocationRepository locationRepository;

    private final AdminHedObjectRepository objectRepository;

    private final AdminHedMajorRepository majorRepository;

    private final CallApiIdentity callApiIdentity;

    private final PortalEventsSession session;

    private final AdminHedEvidenceRepository evidenceRepository;
    ...
    private final AdminHedEventOrganizerRepository eventOrganizerRepository;

    private final LoggerUtil loggerUtil;

    private final EmailUtils emailUtils;

    public AdminHedEventDetailServiceImpl(AdminHedAgendaItemRepository agendaItemRepository, AdminHedEventDetailRepository eventRepository, AdminHewCommentRepository commentRepository, AdminHedResourceRepository resourceReposiotry, AdminHedLocationRepository locationRepository, @Qualifier(PeriodicEventRepository.NAME) PeriodicEventRepository periodicEventRepository, AdminHedObjectRepository objectRepository, EmailUtils emailUtils, AdminHedMajorRepository majorRepository, AdminHedEventOrganizerRepository eventOrganizerRepository, LoggerUtil loggerUtil, CallApiIdentity callApiIdentity, PortalEventsSession session, AdminHedEvidenceRepository evidenceRepository, @Qualifier(PeriodicEventMajorRepository.NAME) PeriodicEventMajorRepository periodicEventMajorRepository, @Qualifier(PeriodicEventObjectRepository.NAME) PeriodicEventObjectRepository periodicEventObjectRepository) {
    ...
    @Transactional
    @Override
    public AdminHedEventDetailApprovedCustom getDetailEventApproved(String idEvent) {
        // Get detail event by id event
        AdminHedEventDetailApprovedResponse eventDetail = eventRepository.getDetailApprovedById(idEvent).get();
        ...
        AdminHedEventDetailApprovedCustom eventDetailCustom = new AdminHedEventDetailApprovedCustom();
        ...
        if (eventDetail.getStatus() == 3) {
            ...
            eventOrganizerList.forEach(item -> {
                List<AdminHedEventOverlapOrganizer> listEventOverlap = eventRepository.getEventOverlapOrganizer(idEvent, eventDetail.getStartTime(), eventDetail.getEndTime());
     ...
     */
    @Override
    public List<AdminHedAllCommentResponse> getComment(String eventId, int pageNumber) {
        ...
        List<AdminHedAllCommentResponse> listComment = new ArrayList<>();
        List<AdminHedReplyCommentResponse> listReplyComment = new ArrayList<>();
        ...
        for (Comment comment : commentFromDB) {
            AdminHedAllCommentResponse commentResponse = new AdminHedAllCommentResponse();
        ...
        List<AdminHedCommentResponse> listReply = commentRepository.getReplyCommentByReplyIdAndEventId(eventId, listReplyCommentId);
        listReply.forEach(comment -> {
            AdminHedReplyCommentResponse replyCommentResponse = new AdminHedReplyCommentResponse();
        ...
        if (lstUserIdToGet.size() > 0) {
            ...
            listComment.forEach(comment -> {
                ...
                if (comment.getListReply() != null) {
                    for (AdminHedReplyCommentResponse replyComment : comment.getListReply()) {
     ...
     */
    @Override
    public List<AdminHewEvidenceResponse> getEvidenceByIdEvent(String idEvent) {
    ...
    public Boolean approverPeriodicEvent(String idEvent) {
        ...
        List<AdminHedEventMajorResponse> eventMajorList = eventRepository.getAllEventMajorByIdEvent(eventFind.get().getId());
        List<AdminHedEventObjectResponse> eventObjectList = eventRepository.getAllEventObjectByIdEvent(eventFind.get().getId());
     ...
     */
    @Override
    public AdminHedAllCommentResponse postComment(AdminHedPostCommentRequest req) {
        ...
        SimpleDateFormat dateFormat = new SimpleDateFormat("HH:mm dd/MM/yyyy");
        AdminHedAllCommentResponse newComment = new AdminHedAllCommentResponse();
     ...
     */
    @Override
    public Boolean deleteComment(AdminHedDeleteCommentRequest req) {
        req.setUserId(session.getCurrentIdUser());
     ...
     */
    @Override
    public List<AdminHedAgendaItemCustom> getListAgendaItemByEventId(String eventId) {
        List<AdminHedAgendaItemsDetailResponse> agendaItemByEventId = agendaItemRepository.getListAgendaItemByEventId(eventId);
        List<String> listIdOrganizer = agendaItemByEventId.stream()
                .map(AdminHedAgendaItemsDetailResponse::getOrganizerId)
                .filter(Objects::nonNull)
                .distinct()
                .collect(Collectors.toList());
        List<SimpleResponse> responseOrganizer = callApiIdentity.handleCallApiGetListUserByListId(listIdOrganizer);
        List<AdminHedAgendaItemCustom> oedAgendaItemCustoms = new ArrayList<>();
        for (AdminHedAgendaItemsDetailResponse xx : agendaItemByEventId) {
            AdminHedAgendaItemCustom agendaItemCustom = new AdminHedAgendaItemCustom();
     ...
     */
    @Override
    public AdminHedReplyCommentResponse replyComment(AdminHedReplyCommentRequest req) {
        req.setUserId(session.getCurrentIdUser());
        ...
        SimpleDateFormat dateFormat = new SimpleDateFormat("HH:mm dd/MM/yyyy");
        AdminHedReplyCommentResponse commentResponse = new AdminHedReplyCommentResponse();
     ...
     */
    @Override
    public List<AdminHedResourceEventResponse> getResourcesByEventId(String eventId) {
     ...
     */
    @Override
    public List<AdminHedLocationEventResponse> getLocationByEventId(String idevent) {
     ...
     */
    @Override
    public List<AdminHedObjectEventResponse> getObjectByEventId(String idevent) {
     ...
     */
    @Override
    public List<AdminHedMajorResponse> getMajorByIdEvent(String id) {
    ...
    @Transactional
    @Override
    public ResponseObject approvalEvent(AdminHewEventApproveRequest req) {
        ...
        if (event.getEventType() == EventType.LECTURER_EVENT) {
            ...
            List<AdminHedLocationEventResponse> locationList = locationRepository.getLocationByEventId(event.getId());
            List<AdminHedResourceEventResponse> resourceList = resourceReposiotry.getResourcesByEventId(event.getId());
            ...
            if (locationList != null) {
                for (AdminHedLocationEventResponse item : locationList) {
            ...
            if (resourceList != null) {
                ...
                for (AdminHedResourceEventResponse item : resourceList) {
