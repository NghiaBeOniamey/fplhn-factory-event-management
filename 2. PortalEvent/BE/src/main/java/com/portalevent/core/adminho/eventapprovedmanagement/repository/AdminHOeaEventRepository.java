package com.portalevent.core.adminho.eventapprovedmanagement.repository;
...
@Repository
public interface AdminHOeaEventRepository extends EventRepository {
            ...
            """, countQuery = """
                select count(e.id)
                     ...
                     where ( :#{#req.name} is null or e.name like :#{'%'+#req.name+'%'})
                       ...
                       and (:#{#request.currentTrainingFacilityCode} is null or e.training_facility_code like :#{#request.currentTrainingFacilityCode})
            """, nativeQuery = true)
    Page<AdminHOeaEventApprovedResponse> getListEventAppoved(Pageable pageable, AdminHOeaEventApprovedRequest req, TokenFindRequest request);
