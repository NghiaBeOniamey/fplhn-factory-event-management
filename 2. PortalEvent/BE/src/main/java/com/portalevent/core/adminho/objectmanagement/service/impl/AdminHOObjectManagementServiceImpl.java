package com.portalevent.core.adminho.objectmanagement.service.impl;
...
import com.portalevent.core.adminho.objectmanagement.repository.AdminHOObjectManagementRepository;
import com.portalevent.core.adminho.objectmanagement.service.AdminHOObjectManagementService;
...
@Service
public class AdminHOObjectManagementServiceImpl implements AdminHOObjectManagementService {

    private final AdminHOObjectManagementRepository repository;

    private final LoggerUtil loggerUtil;

    public AdminHOObjectManagementServiceImpl(AdminHOObjectManagementRepository repository, LoggerUtil loggerUtil) {
     ...
     */
    @Override
    public PageableObject<AdminHOObjectManagementListResponse> getListObject(AdminHOObjectManagementListRequest request) {
        request.setName(request.getName().replaceAll("\\s+", " "));
        return new PageableObject<AdminHOObjectManagementListResponse>(repository.getObjectList(PageRequest.of(request.getPage(),
     ...
     */
    @Override
    public Object postObject(@Valid AdminHOObjectManagementCreatRequest request) {
        request.setName(request.getName().replaceAll("\\s+", " "));
        if (request.getName().length() >= 100) {
            throw new RestApiException(Message.OBJECT_NAME_LESS_THAN_100_CHARACTERS);
        }
        List<AdminHOObjectManagementListResponse> objectListResponses = repository.findNameObject(request.getName());
     ...
     */
    @Override
    public Object updateObject(String id, @Valid AdminHOObjectManagementCreatRequest request) {
        request.setName(request.getName().replaceAll("\\s+", " "));
