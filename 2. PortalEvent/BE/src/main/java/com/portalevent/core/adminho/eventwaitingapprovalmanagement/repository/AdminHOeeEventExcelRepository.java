...
public interface AdminHOeeEventExcelRepository extends EventRepository {
    ...
    @Query(value = """
            ...
            WHERE
             e.status != 1
             AND e.semester_id = :#{#request.idSemester}
            AND c.campus_id = :#{#request.idCampus}
            AND d.department_id = :#{#request.idDepartment}
            GROUP BY
             c.name,
             d.name,
             e.name,
